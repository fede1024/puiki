(ns connect.email
 ...
 (:import
   [javax.mail Session Message Authenticator PasswordAuthentication Transport Message$RecipientType Folder Flags$Flag]
   [javax.mail.internet MimeMessage InternetAddress]
   [java.util Properties]))

(defn set-email! [& {:keys [username password server port protocol ssl] :as data}]
  (update! :admin {:_id "email"}
    (merge data {:_id "email"})))
...
(defn html-email [recipients subject text]
  (let [{:keys [session username]} *session*]
    (doto (MimeMessage. session)
      (.setFrom (InternetAddress. username))
      (.setRecipients rTO (into-array (map #(InternetAddress. %) recipients)))
...
(defn send-mail [address subject body]
  (let [{:keys [username password server port protocol ssl]} (get-email)]
    (with-session username password server port protocol ssl
      (send-email
        (html-email [address] subject body)))))

;(send-mail "giraud.federico@gmail.com" "Prova"
;  "Questa è una prova")
