...
;  (let [votes (:votes post)
;        my-vote (if (current-id)
;                  (or (get (:votes post) (current-id)) 0)
;                  0)
;        vote (apply + (vals votes))]
;    [:span.vote
;     (when (and (current-id) (< my-vote 1))
;       (link-to (post-vote-path post 'up)
;         [:img {:height 16 :src "/images/up.png"}]))
;     (when (and (current-id) (> my-vote -1))
;       (link-to (post-vote-path post 'down)
;         [:img {:height 16 :src "/images/down.png"}]))
;     [:span.voteValue vote]])
  "todo")
...
(def new-post-errors 
  {:title "Titolo non valido" :content "Contenuto non valido"
